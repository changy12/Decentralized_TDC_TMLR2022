...
class DTD_cliff:
    def __init__(self,seed_init=1,state_space=None,action_spaces=None,\
                 ...
                 target_policy=None, V=None, features='identity matrix'):
        set_seed(seed_init)
        ...
        if type(features) is str:
            ...
            DAC_dict['num_V_features']=self.features.shape[0]
    ...
    def sim1(self,hyp,init_Theta=None,init_W=None,seed_sim=None):
        ...
        # self.Theta_star=np.zeros(shape=(num_totaliters+1,self.num_features))
        #Among the optimal theta values, Theta_star[t,:] is the closest to Theta_avg[t,:]
        ...
        for hyp in hyps:
            ...
            if (result_type in self.results[hyp_str].keys()):
                ...
                if result_type=='Theta_avg_err':
                    result_preplot=np.sqrt(self.results[hyp_str][result_type][:,hyp['plot_iters']])
                else:
                    result_preplot=np.sqrt(self.results[hyp_str][result_type][:,hyp['plot_iters'],:].max(axis=2))
    ...
    #     except np.linalg.LinAlgError: 
    #         return -np.linalg.pinv(self.C).dot(self.A.dot(theta)+self.b.mean(axis=1))
